...
-- @treturn Animation the new Animation.
...
function Animation:update()
  if self.current_frame == self.sprites[self.current_sprite].end_frame then
    self.current_sprite = self.current_sprite == #self.sprites
        and 1 or self.current_sprite + 1
  end
...
function Animation:__tostring()
  return string.format("Animation = {current_sprite = %d/%d, " ..
      "current_frame = %d/%d}", self.current_sprite, #self.sprites,
